...
use crate::{
    ...
    engine::{linear_search, ConstraintSatisfactionSolver, SATEngineDataStructures},
...
impl Pumpkin {
    ...
    pub fn solve(&mut self) -> PumpkinExecutionFlag {
        ...
        match csp_execution_flag {
            CSPSolverExecutionFlag::Timeout => return PumpkinExecutionFlag::Timeout,
            CSPSolverExecutionFlag::Feasible => {}
            CSPSolverExecutionFlag::Infeasible => return PumpkinExecutionFlag::Infeasible,
            _ => unreachable!(),
        };

        if !self.objective_function.is_empty() {
            let optimal_solution = linear_search::solve(
                &mut self.csp_solver,
